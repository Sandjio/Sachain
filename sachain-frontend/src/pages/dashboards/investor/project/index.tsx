...
// import { InvestorProjectDetail } from '@/features/project/components/InvestorProjectDetail';
...
// import { InvestmentFlow } from '@/features/project/components/InvestmentFlow';
...
//   //const [activeView, setActiveView] = useState<'list' | 'investment'>('list');
//     const [activeView, setActiveView] = useState<'list' | 'details' | 'investment'>('list');
...
//       (priceFilter === '50-100' &&
//         project.pricePerStock >= 50 &&
//         project.pricePerStock <= 100) ||
...
//   // Switch to investment view with selected project
//   const handleInvestNow = (project: APIProject) => {
//     setSelectedProject(project);
//     setActiveView('investment');
//   };

//   // Return to project list view after investment or cancel
...
//           Discover and invest in promising startups that have completed
//           tokenization
//         </p>
//       </div>
...
//               {filteredProjects.length} investment
...
//           onInvestComplete={handleInvestmentComplete}
//         />
//       )}

//       {activeView === 'investment' && selectedProject && (
//         <InvestmentFlow
...
export default function InvestorProjectsPage() {
  ...
  const hasActiveFilters = useMemo(() => {
    return searchQuery !== '' || categoryFilter !== 'all' || priceFilter !== 'all';
  }, [searchQuery, categoryFilter, priceFilter]);

  // Memoize callback handlers
  const handleInvestNow = useCallback((project: APIProject) => {
    setSelectedProject(project);
    setActiveView('investment');
  }, []);

  const handleViewDetails = useCallback((project: APIProject) => {
    setSelectedProject(project);
    setActiveView('details');
  }, []);
  ...
  const handleInvestmentComplete = useCallback(() => {
    setSelectedProject(null);
    setActiveView('list');
    // Optionally refetch to update data
    refetch();
  }, [refetch]);
