...
export function InvestorDashboard({ 
  ...
  const { balance, tokens, loading, error } = useHederaBalance(walletAddress, refreshTrigger);
    ...
    <div className="space-y-8 pb-8">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          Track your portfolio and discover new investment opportunities on Hedera network
        </p>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex items-center gap-1 text-xs text-emerald-600">
                ...
                <span>{formatPercentage(portfolioSummary.totalReturnPercentage)}</span>
              </div>
            </div>
            <div>
        ...
        <Card className="bg-white border border-gray-200 hover:border-[#90A5FB]/50 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300">
          <CardContent className="p-6">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">{portfolioSummary.activeInvestments}</p>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              {userInvestments.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 mb-6 max-w-sm mx-auto">
                    Start building your portfolio by exploring available projects on the Hedera network.
                  </p>
                  <Button 
                ...
                <div className="space-y-4">
                  {userInvestments.map((investment) => (
                    <div key={investment.id} className="flex items-center space-x-4 p-4 bg-gradient-to-br from-blue-50/50 to-indigo-50/30 border border-blue-100 rounded-xl hover:shadow-md transition-shadow">
                      <ImageWithFallback 
                        src={investment.image || '/placeholder-project.jpg'}
                        alt={investment.projectName}
                        className="w-16 h-16 rounded-lg object-cover"
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between">
                          <div>
                            <h3 className="font-semibold text-gray-900">{investment.projectName}</h3>
                            <p className="text-sm text-gray-600 capitalize">{investment.category}</p>
                            <p className="text-xs text-gray-500 mt-1">{investment.tokensOwned} tokens</p>
                          </div>
                          <div className="text-right">
                            <p className="font-semibold text-gray-900">{formatCurrency(investment.currentValue)}</p>
                            <p className={`text-sm ${
                              investment.currentValue >= investment.amountInvested ? 'text-emerald-600' : 'text-red-600'
                            }`}>
                              {formatPercentage(((investment.currentValue - investment.amountInvested) / investment.amountInvested) * 100)}
                            </p>
                            <Badge className={getStatusColor(investment.status)}>
          ...
          <Card className="bg-white border border-gray-200 rounded-xl shadow-sm">
            ...
            <CardContent className="pt-6">
              {featuredProjects.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {featuredProjects.map((project) => (
                    <Card key={project.projectId} className="bg-white border border-gray-200 hover:border-[#90A5FB]/50 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
                      ...
                      <CardContent className="p-4">
                        ...
                        <p className="text-sm text-gray-600 capitalize mb-3">{project.category}</p>
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-gray-500">per token</span>
                        </div>
                      </CardContent>
                    </Card>
                  ))}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-white/50 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">Available Projects</span>
                  </div>
                ...
                <div className="flex items-center justify-between p-3 bg-white/50 rounded-lg">
                  <div className="flex items-center gap-2">
                    <DollarSign className="h-4 w-4 text-emerald-600" />
                    <span className="text-sm text-gray-700">Avg. Token Price</span>
                  </div>
  ...
  {walletAddress && (
    <div>
      ...
      {!loading && !error && (
        <p>HBAR Balance: {balance !== null ? balance.toFixed(6) : 'N/A'} ℏ</p>
      )}

      <button onClick={() => setRefreshTrigger((t) => t + 1)}>Refresh Balance</button>
    </div>
  )}
</CardContent>
